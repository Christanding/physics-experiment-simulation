...
from dm import 生成锁相仿真数据, 相位包裹到_pi, 运行数字锁相, 锁相仿真配置
...
def enrich_run_for_plot(run: dict[str, Any], cfg: 锁相仿真配置) -> None:
    run["theoretical_peak_v"] = cfg.目标幅值_A
    run["true_phase_deg"] = math.degrees(相位包裹到_pi(cfg.目标相位_phi))
    run["figure_dpi"] = 220
...
def run_case(
    ...
    input_vpp: float,
    case_index: int,
    phase_rad: float = PHI_TRUE_RAD,
    figure_root: Path | None = None,
    figure_prefix: str = "simulation",
) -> dict[str, Any]:
    ...
    phi_rec = safe_float(run["phi_true_rec"])
    u_phi = safe_float(run["u_phi"])
    amp_error_percent = 100.0 * (float(run["a_rec"]) / cfg.目标幅值_A - 1.0)
    phase_error_deg = None if phi_rec is None else math.degrees(abs(相位包裹到_pi(phi_rec - cfg.目标相位_phi)))
    nnear = run["nnear检测结果"]

    return {
        "frequency_hz": f0_hz,
        "input_vpp_generator": input_vpp,
        "theoretical_output_peak_mV": 1e3 * cfg.目标幅值_A,
        "recovered_peak_mV": 1e3 * float(run["a_rec"]),
        "amplitude_error_percent": amp_error_percent,
        "u_A_mV": 1e3 * float(run["u_a"]),
        "relative_u_A_percent": 100.0 * float(run["u_a"]) / float(run["a_rec"]),
        "true_phase_deg": math.degrees(相位包裹到_pi(cfg.目标相位_phi)),
        "recovered_phase_deg": None if phi_rec is None else math.degrees(phi_rec),
        "phase_error_deg": phase_error_deg,
        "u_phi_deg": None if u_phi is None else math.degrees(u_phi),
        "phase_valid": bool(run["phase_valid"]),
        "input_snr_db": measurement_input_snr_db(components),
        "lockin_snr_db": safe_float(run["snr_vector_db"]),
        ...
        "near_frequency_detected": bool(nnear["检测到显著拍频峰"]),
        "near_frequency_R": float(nnear["峰中位数比值_R"]),
...
def run_phase_sweep() -> list[dict[str, Any]]:
    ...
    for i, phase_deg in enumerate(PHASE_SWEEP_DEGREES):
        rows.append(run_case(
            ...
            figure_prefix="double_phase_sweep",
        ))
    return rows
...
def config_snapshot() -> dict[str, Any]:
    ...
    return {
        ...
        "algorithm_switches": {
            ...
            "processing": asdict(cfg.处理项),
        },
    }


def write_summary_md(rows: list[dict[str, Any]], phase_rows: list[dict[str, Any]]) -> Path:
    abs_errors = [abs(float(row["amplitude_error_percent"])) for row in rows]
    rel_uncertainties = [float(row["relative_u_A_percent"]) for row in rows]
    phase_errors = [float(row["phase_error_deg"]) for row in rows if row["phase_error_deg"] is not None]
    worst = max(rows, key=lambda row: abs(float(row["amplitude_error_percent"])))
    ...
    lines = [
        ...
        "",
        f"仿真设置：频率 `1/2/5/10 kHz`，信号源档位 `0.1/1/3/5/10 Vpp`，理论输出峰值按 `信号源 Vpp / 20` 计算；每组采样 `T={SIM_DURATION_S:.1f} s`，每周期 {SAMPLES_PER_CYCLE} 点。本版为强噪声压力测试：测量通道白噪声按 `e_n={MEASUREMENT_WHITE_EN_V_PER_SQRT_HZ:.2e} V/sqrt(Hz)` 和 `sigma=e_n sqrt(fs/2)` 标定，参考通道白噪声按 `e_n={REFERENCE_WHITE_EN_V_PER_SQRT_HZ:.2e} V/sqrt(Hz)` 标定，并叠加工频、慢漂移、1/f 噪声和邻近频率干扰。",
        "",
        "## 总体结果",
        "",
        f"- 共完成 {len(rows)} 组加噪声仿真，全部生成 6 联图。",
        f"- 幅值绝对偏差：中位数 {np.median(abs_errors):.3f}%，最大 {max(abs_errors):.3f}%。",
        f"- 相对幅值不确定度：中位数 {np.median(rel_uncertainties):.3f}%，最大 {max(rel_uncertainties):.3f}%。",
        f"- 相位误差：中位数 {np.median(phase_errors):.3f}°，最大 {max(phase_errors):.3f}°。",
        f"- 最大幅值偏差出现在 {worst['frequency_hz']/1000:g} kHz、{worst['input_vpp_generator']:g} Vpp：偏差 {worst['amplitude_error_percent']:+.3f}%。",
        "",
        "## 恢复效果与不确定度",
        "",
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"{row['theoretical_output_peak_mV']:.3f} mV | {row['recovered_peak_mV']:.3f} mV | "
            f"{format_percent(row['amplitude_error_percent'], 3)} | {row['u_A_mV']:.4f} mV | "
            f"{row['relative_u_A_percent']:.4f}% | {row['true_phase_deg']:.2f}° | "
            f"{fmt_number(row['recovered_phase_deg'], 3)}° | {fmt_number(row['u_phi_deg'], 4)}° | "
            f"{fmt_number(row['lockin_snr_db'], 2)} dB | [{Path(figure).name}]({figure}) |"
        )
    ...
    for row in phase_rows:
        lines.append(
            f"| {row['true_phase_deg']:.0f}° | {row['theoretical_output_peak_mV']:.3f} mV | "
            f"{row['recovered_peak_mV']:.3f} mV | {row['amplitude_error_percent']:+.3f}% | "
            f"{row['u_A_mV']:.4f} mV | {row['relative_u_A_percent']:.4f}% | "
            f"{fmt_number(row['recovered_phase_deg'], 3)}° | {fmt_number(row['phase_error_deg'], 3)}° | "
            f"{fmt_number(row['u_phi_deg'], 4)}° | "
...
def main() -> None:
    ...
    csv_path = DATA_DIR / "noisy_algorithm_simulation_results.csv"
    phase_csv_path = DATA_DIR / "double_phase_sweep_results.csv"
    config_path = OUT_DIR / "simulation_config.json"
